...
const Login = () => {
  ...
  const { login } = useAuth();
  const { getPharmacyForUser, setSelectedPharmacy } = usePharmacy();
    ...
    if (result.success) {
      ...
      setTimeout(() => {
        if (role === 'admin') {
          ...
          const pharmacy = getPharmacyForUser(result.user);
          if (pharmacy) {
            setSelectedPharmacy(pharmacy);
            navigate('/pharmacy-dashboard', { state: { pharmacy } });
          } else {
  ...
  // Show only spinner when loading (no text)
  if (loading) {
